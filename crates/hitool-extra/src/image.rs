...
/// Parses Hutool-style format names (`png` / `jpg` / `jpeg` / `webp`).
...
mod tests {
    ...
    #[test]
    fn img_util_scale_cut_convert() {
        let scaled = ImgUtil::scale(&fixture(), 5, 5, OutputFormat::Png).unwrap();
        assert_eq!(ImgUtil::read_size(&scaled).unwrap(), (5, 3));
        let cut = ImgUtil::cut(&fixture(), 2, 1, 4, 3, OutputFormat::Png).unwrap();
        assert_eq!(ImgUtil::read_size(&cut).unwrap(), (4, 3));
        let jpeg = ImgUtil::convert(&fixture(), ImgUtil::IMAGE_TYPE_JPG).unwrap();
        assert!(jpeg.len() > 16);
    }
}
