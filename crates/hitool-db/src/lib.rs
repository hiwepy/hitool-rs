...
mod tests {
    ...
    async fn postgres_pool_executes_real_transactional_sql() {
        ...
        .await
        .unwrap();
        sqlx::query("CREATE TEMP TABLE hitool_item (id BIGSERIAL PRIMARY KEY, name TEXT NOT NULL)")
            .execute(&pool)
            .await
            .unwrap();
        let mut transaction = pool.begin().await.unwrap();
        sqlx::query("INSERT INTO hitool_item (name) VALUES ($1)")
            .bind("hitool")
            .execute(&mut *transaction)
            .await
            .unwrap();
        transaction.commit().await.unwrap();
        let count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM hitool_item")
            .fetch_one(&pool)
            .await
            .unwrap();
        assert_eq!(count.0, 1);
    ...
    async fn mysql_pool_executes_real_transactional_sql() {
        ...
        .unwrap();
        sqlx::query(
            "CREATE TEMPORARY TABLE hitool_item (id BIGINT AUTO_INCREMENT PRIMARY KEY, name TEXT NOT NULL)",
        )
        .execute(&pool)
        .await
        .unwrap();
        let mut transaction = pool.begin().await.unwrap();
        sqlx::query("INSERT INTO hitool_item (name) VALUES (?)")
            .bind("hitool")
            .execute(&mut *transaction)
            .await
            .unwrap();
