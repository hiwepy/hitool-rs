...
use hitool_core::annotation::fixtures::types;
...
fn element_annotation_scanner_scan_test() {
    ...
    let (_ty, field, _method) = fixtures::scanner_example(&mut reg);
    let scanner = ElementAnnotationScanner;
    let mut map: HashMap<i32, Vec<Arc<hitool_core::annotation::AnnotationMirror>>> = HashMap::new();
    {
        let map_ref = &mut map;
        let mut consumer: hitool_core::annotation::scanner::annotation_scanner::ScanConsumer<'_> =
