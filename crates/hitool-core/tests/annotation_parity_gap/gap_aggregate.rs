...
fn generic_synthesized_aggregate_annotation_synthetic_annotation_test() {
    ...
    assert_eq!(
        ...
        child_syn
            ...
            .map(|v| match v {
                hitool_core::annotation::AnnotationValue::Class(c) => c.clone(),
                _ => String::new(),
            })
            .unwrap()
    );
...
fn generic_synthesized_aggregate_annotation_alias_for_test() {
    ...
    let ann2 = reg.annotation(
        ...
        HashMap::from([(
            "value".to_string(),
            hitool_core::annotation::AnnotationValue::String("Foo".into()),
        )]),
    );
...
fn generic_synthesized_aggregate_annotation_alias_for_and_mirror_test() {
    ...
    let ann = reg.annotation(
        ...
        HashMap::from([(
            ...
            hitool_core::annotation::AnnotationValue::String("test".into()),
        )]),
    );
    let aggregate = GenericSynthesizedAggregateAnnotation::new(ann);
...
fn cacheable_synthesized_annotation_attribute_processor_get_attribute_value_test() {
    use hitool_core::annotation::TestValueSynthesizedAnnotation;

    use hitool_core::annotation::SynthesizedAnnotationAttributeProcessor;

    let processor = CacheableSynthesizedAnnotationAttributeProcessor::new();
    let a1: Arc<dyn hitool_core::annotation::SynthesizedAnnotation> =
        ...
        HashMap::from([
            (
                "name".to_string(),
                hitool_core::annotation::AnnotationValue::String("name1".into()),
            ),
            (
                "value".to_string(),
                hitool_core::annotation::AnnotationValue::I32(111),
            ),
        ]),
    );
    let a2: Arc<dyn hitool_core::annotation::SynthesizedAnnotation> =
        TestValueSynthesizedAnnotation::new(
        0,
        0,
        HashMap::from([
            (
                "name".to_string(),
                hitool_core::annotation::AnnotationValue::String("name2".into()),
            ),
            (
                "value".to_string(),
                hitool_core::annotation::AnnotationValue::String("value2".into()),
            ),
        ]),
    );
    let list = vec![a1, a2];
    ...
    assert_eq!(
        ...
        processor
            ...
            .and_then(|v| match v {
                hitool_core::annotation::AnnotationValue::I32(i) => Some(i),
                _ => None,
            })
            .unwrap()
    );
