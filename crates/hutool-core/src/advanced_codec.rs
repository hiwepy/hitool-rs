...
mod tests {
    ...
    fn base58_and_check_preserve_leading_zeroes() {
        ...
        let checked = base58_encode_checked(Some(0), b"hutool");
        assert_eq!(base58_decode_checked(&checked, true).unwrap(), b"hutool");
        assert_eq!(base58_decode_checked_auto(&checked).unwrap(), b"hutool");
        assert_eq!(
    ...
    fn hashids_support_numbers_hex_and_custom_alphabet() {
        let codec = HashIds::new("hutool", 12).unwrap();
