...
//!
//! 覆盖更多 hutool 测试场景，提升 hutool-rs 测试覆盖率。
...
#[test]
fn v01() { assert!(VersionUtil::is_less_than_str("1.0.0", "2.0.0")); }
#[test]
fn v02() { assert!(VersionUtil::is_greater_than_str("2.0.0", "1.0.0")); }
#[test]
fn v03() { assert!(VersionUtil::is_greater_than_or_equal_str("1.0.0", "1.0.0")); }
...
#[test]
fn f03() { let p = "/tmp/test_coverage_boost.txt"; FileUtil::write_utf8_string(p, "test").unwrap(); assert_eq!(FileUtil::read_utf8_string(p).unwrap(), "test"); FileUtil::delete(p).unwrap(); }
