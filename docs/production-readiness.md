...
| Complete Hutool functional parity | in progress | pinned v5.8.46 inventory contains 13,871 public production API records; 432 records have implementation and executable evidence: all 175 `core.codec` records plus 257 reviewed `core.collection` records |
...
| 100% test coverage | in progress | current all-feature baseline: lines 87.00%, regions 87.45%, functions 85.59%; new collection type, partition, iterator, adapter, stream, and utility code is 100% across all three metrics and CI requires 100% workspace-wide |
