...
| Complete Hutool functional parity | in progress | pinned v5.8.46 inventory contains 13,871 public production API records; 412 records have implementation and executable evidence: all 175 `core.codec` records plus 237 reviewed `core.collection` records |
...
| 100% test coverage | in progress | current all-feature baseline: lines 86.58%, regions 87.18%, functions 85.25%; new collection type, partition, iterator, adapter, and utility code is 100% across all three metrics and CI requires 100% workspace-wide |
