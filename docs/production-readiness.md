...
| Complete Hutool functional parity | in progress | pinned v5.8.46 inventory contains 13,871 public production API records; 685 records have implementation and executable evidence: all 175 `core.codec`, 422 `core.collection`, 9 `core.getter`, 10 `core.clone`, 33 `core.compiler`, and 36 `core.stream` records |
...
| 100% test coverage | in progress | current all-feature baseline: lines 90.00%, regions 90.35%, functions 88.97%; new collection, getter, clone, compiler, and stream implementations are 100% across all three metrics and CI requires 100% workspace-wide |
