...
| 100% test coverage | in progress | current all-feature baseline: lines 98.57% (26,235/26,616), regions 98.54% (42,951/43,587), functions 98.09% (3,648/3,719); the facade registry, JWT, cache, system, cron, setting, AI, log, JSON, and HTTP crates are each exactly 100%, `hitool-core/src/builder.rs`, `mutable.rs`, `boolean_util.rs`, and `hash_util.rs` are each exactly 100%, with the new boolean/hash modules contributing 550 lines, 1,017 regions, and 87 functions; CI still requires 100% workspace-wide |
