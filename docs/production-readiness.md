...
| 100% test coverage | in progress | current all-feature baseline: lines 98.59% (26,527/26,906), regions 98.56% (43,495/44,129), functions 98.14% (3,696/3,766); the facade registry, JWT, cache, system, cron, setting, AI, log, JSON, and HTTP crates are each exactly 100%, `hitool-core/src/builder.rs`, `mutable.rs`, `boolean_util.rs`, `hash_util.rs`, and `hex_util.rs` are each exactly 100%, with the new HexUtil module contributing 290 covered lines, 542 covered regions, and 47 covered functions; CI still requires 100% workspace-wide |
