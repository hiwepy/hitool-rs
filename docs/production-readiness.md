...
| 100% test coverage | in progress | current all-feature baseline: lines 98.68% (28,205/28,583), regions 98.66% (46,521/47,154), functions 98.25% (3,923/3,993); the facade registry, JWT, cache, system, cron, setting, AI, log, JSON, and HTTP crates are each exactly 100%, `hitool-core/src/builder.rs`, `mutable.rs`, `boolean_util.rs`, `byte_util.rs`, `charset_util.rs`, `coordinate_util.rs`, `credit_code_util.rs`, `hash_util.rs`, `hex_util.rs`, `page_util.rs`, `phone_util.rs`, `radix_util.rs`, and `version_util.rs` are each exactly 100%, with the three new utility modules contributing 653 covered lines, 1,243 covered regions, and 86 covered functions; CI still requires 100% workspace-wide |
