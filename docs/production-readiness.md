...
| 100% test coverage | in progress | current all-feature baseline: lines 98.61% (26,885/27,263), regions 98.59% (44,115/44,748), functions 98.17% (3,749/3,819); the facade registry, JWT, cache, system, cron, setting, AI, log, JSON, and HTTP crates are each exactly 100%, `hitool-core/src/builder.rs`, `mutable.rs`, `boolean_util.rs`, `byte_util.rs`, `hash_util.rs`, and `hex_util.rs` are each exactly 100%, with the new ByteUtil module contributing 357 covered lines, 619 covered regions, and 53 covered functions; CI still requires 100% workspace-wide |
