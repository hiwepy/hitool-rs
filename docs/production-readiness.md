...
| 100% test coverage | in progress | current all-feature baseline: lines 98.65% (27,552/27,930), regions 98.62% (45,278/45,911), functions 98.21% (3,837/3,907); the facade registry, JWT, cache, system, cron, setting, AI, log, JSON, and HTTP crates are each exactly 100%, `hitool-core/src/builder.rs`, `mutable.rs`, `boolean_util.rs`, `byte_util.rs`, `charset_util.rs`, `credit_code_util.rs`, `hash_util.rs`, `hex_util.rs`, `page_util.rs`, and `radix_util.rs` are each exactly 100%, with the four new utility modules contributing 667 covered lines, 1,163 covered regions, and 88 covered functions; CI still requires 100% workspace-wide |
