...
| Complete Hutool functional parity | in progress | pinned v5.8.46 inventory contains 13,871 public production API records; 649 records have implementation and executable evidence: all 175 `core.codec`, 422 `core.collection`, 9 `core.getter`, 10 `core.clone`, and 33 `core.compiler` records |
...
| 100% test coverage | in progress | current all-feature baseline: lines 89.59%, regions 90.02%, functions 88.54%; new collection, getter, clone, and compiler implementations are 100% across all three metrics and CI requires 100% workspace-wide |
