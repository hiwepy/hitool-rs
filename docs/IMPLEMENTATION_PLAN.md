...
> **当前迁移基线**：见 `/Users/wandl/workspaces/workspace-github/hitool-rs/MIGRATION_STATUS.md`（完成度 ≈ 59.6% 文件数 / ≈ 50% 加权方法数）
> **定位**：一比一复刻 Hutool Java 的 Rust 实现，对象/方法/参数命名严格对齐，Rust 端使用 idiomatic API + 成熟 crate 引擎
> **状态**：待批准实施

> ⚠️ **关于 hitool-compat-hutool 的重新定位**：经过对当前实现的诚实审计（仅 114 行 / 2 个 facade / 11 个方法），
> 本文档**早期设想**的「双 API 表面（idiomatic + hutool-compat）」架构目标已被用户明确否决。
> 当前优先级是把 `hitool-db`（缺 75 文件）、`hitool-extra`（缺 170 文件）、`hitool-cron`（缺 37 文件）、
> `hitool-http`（缺 47 文件）、`hitool-crypto`（缺 48 文件）等**主仓核心模块**先迁移到位。
> `hitool-compat-hutool` crate 在主仓迁移完成之前**没有任何迁移价值**（背后无实现可委托）。
...
- [五、hitool-core 内部模块（1:1 对齐 Java 包）](#五hitool-core-内部模块11-对齐-java-包)
- [六、核心类型/Trait 签名（重点 hitool-core）](#六核心类型trait-签名重点-hitool-core)
...
- [附录 B：参考项目](#附录-b参考项目)
- [附录 C：Hutool v5.8.x 模块 → HiTool crate 映射表](#附录-chutool-v58x-模块--hitool-crate-映射表)
- [附录 D：DDD4J → HiTool-Rs 完整生态映射](#附录-dddd4j--hitool-rs-完整生态映射)
...
| **文件路径** | `cn.hutool.core.collection.CollUtil` ↔ `hitool-core/src/collection/coll_util.rs` |
| **对象/方法/参数命名** | 100% 对齐（snake_case 转换除外） |
| **方法逻辑** | 核心逻辑对齐，错误处理 Rust 化（`Result<T, E>`） |
| **hitool-poi** | 按用户说明**只做对象/方法/参数占位**，等待 `easyexcel-rs`/`easydoc-rs`/`easyofd-rs`/`easypdf-rs` 完成 |
| **已有实现** | **不删减**，顺着原思路继续迁移（解决 hitool-core 内部双重路径问题） |
...
本项目参考了 `sa-token-rs`（同模式 Java→Rust 移植，模板成熟）和 `hitool-rs` 现有的 7 条核心设计决策：
...
| 层 | 决策 | 原因 |
|---|---|---|
| `hitool-core` / `hitool-cron` / ... | **Rust idiomatic 单一 API** | 充分利用 Rust 类型系统、所有权、async |
| `hitool-compat-hutool` | **暂时不扩展**，作为**未实现占位** | 底层主仓未完成前，compat 层无真实实现可委托；当前 114 行价值低 |

**实现策略**：每个 Hutool 类直接以 Rust struct/enum + idiomatic API 实现。命名通过 snake_case 转换保留 Java 业务语义：

```rust
// hitool-core/src/collection/coll_util.rs — Rust idiomatic 唯一实现
pub struct CollUtil;
impl CollUtil {
...
> ⚠️ hitool-core 当前存在双重路径（如 `coll_util.rs` 顶层 + `collection/coll_util.rs`），
> 这是历史迁移过渡状态。**原则：保留所有已有实现**，通过 `pub use` 在 lib.rs 重新导出。

```rust
// hitool-core/src/lib.rs
...
hitool-core ──→ (不依赖任何本 workspace crate)

hitool-extra ──→ hitool-core
hitool-http ──→ hitool-core + hitool-json
hitool-db ──→ hitool-core + hitool-json
hitool-cron ──→ hitool-core + hitool-log
hitool-aop ──→ hitool-core

# hitool-compat-hutool 暂时冻结（不扩展、不删除）
# 待主仓迁移完成后再评估
hitool-crypto ──→ hitool-core
hitool-jwt ──→ hitool-crypto + hitool-json
hitool-ai ──→ hitool-http + hitool-json + hitool-core
```
...
| Java | Rust | 示例 |
|---|---|---|
| 包 `cn.hutool.core.collection` | crate + mod `hitool_core::collection` | — |
...
```text
hitool-rs/                                       # 仓库根（对应 Hutool 根 pom.xml）
...
│   │       └── json_object.rs
│   │
│   ├── hitool-core/                             # ★ 核心库（对应 hutool-core/）
│   ├── hitool-aop/                              # AOP trait + interceptor
│   ├── hitool-bloom-filter/                     # BloomFilter + BitMap
│   ├── hitool-cache/                            # Cache (基于 moka)
│   ├── hitool-captcha/                          # 验证码生成
│   ├── hitool-cron/                             # Cron 表达式 + 调度
│   ├── hitool-crypto/                           # 加密（AES-GCM/HMAC/SHA/Argon2id）
│   ├── hitool-db/                               # 数据库（基于 sqlx）
│   ├── hitool-dfa/                              # Aho-Corasick
│   ├── hitool-extra/                            # QR/压缩/邮件/拼音/分词/...
│   ├── hitool-http/                             # HTTP 客户端（基于 reqwest）
│   ├── hitool-json/                             # JSON（基于 serde_json）
│   ├── hitool-jwt/                              # JWT（基于 jsonwebtoken）
│   ├── hitool-log/                              # 日志（基于 tracing）
│   ├── hitool-script/                           # 脚本（基于 rhai）
│   ├── hitool-setting/                          # 配置（基于 config）
│   ├── hitool-socket/                           # TCP/UDP（基于 tokio）
│   ├── hitool-system/                           # OS/进程信息（基于 sysinfo）
│   ├── hitool-ai/                               # AI Provider 抽象
│   ├── hitool-macros/                           # proc-macro 工具
│   ├── hitool-compat-hutool/                    # 🟡 暂时冻结（114 行占位，待主仓完成后再评估）
│   ├── hitool-test-support/                     # 测试工具
│   │
│   └── hitool-poi/                              # 🟡 占位骨架（按用户要求暂未实现）
│
├── examples/                                    # 跨 crate 示例
├── benches/                                     # 性能基准
...
新增/改动（按用户优先级）：
- **`crates/hitool-extra/`** 扩充（**缺 170 文件，最高优先级**）
- **`crates/hitool-db/`** 扩充（**缺 75 文件**）
- **`crates/hitool-crypto/`** 扩充（缺 48 文件）
- **`crates/hitool-http/`** 扩充（缺 47 文件）
- **`crates/hitool-cron/`** 扩充（缺 37 文件）
- **`crates/hitool-poi/`** 创建（用户要求占位实现）
- `crates/hitool-core/tests/` 扩充（330 Java 测试 → 目标 200+ Rust 测试）
- `crates/hitool-compat-hutool/` **冻结维持现状**（114 行，待主仓完成后再评估）
...
hitool-core ──→ (零依赖，仅 std + 第三方 crate)

# hitool-compat-hutool 暂时冻结，不扩展、不删除、不新增依赖
```

---

## 五、hitool-core 内部模块（1:1 对齐 Java 包）
...
// hitool-core/src/collection/mod.rs
mod coll_util;
...
// ❌ 错误示例：实现代码堆在顶层
// hitool-core/src/coll_util.rs        ← 违反规则 1
// hitool-core/src/iter_util.rs        ← 违反规则 1
// hitool-core/src/lib.rs (280 行)     ← 违反规则 2
```

### 5.0.1 🎯 1:1 全文件覆盖硬性要求（**用户最终要求**）

> "hutool 中的除了 hutool-poi，每一个模块，每一个文件在 hitool-rs 中必须有对应的实现。"
...
| **hitool-poi 唯一例外** | 按用户原话"hutool 中的除了 hutool-poi"，hitool-poi 暂保留占位 |
...
# 2. 列出 hitool-rs 全部 .rs 文件
find /Users/wandl/workspaces/workspace-github/hitool-rs/crates -name "*.rs" \
  -not -path "*/target/*" -not -path "*/tests/*" -not -path "*/examples/*" \
  | grep -v "/hitool-poi/" > /tmp/hitool_all_rs.txt
...
| Phase | 验收门槛 | 说明 |
|---|---|---|
| Phase 1 结束 | ≥ 30% | hitool-extra + hitool-core 目录重组完成 |
| Phase 2 结束 | ≥ 60% | hitool-cron / hitool-db 大件补齐 |
| Phase 3 结束 | ≥ 85% | hitool-crypto / hitool-http 补齐 |
| Phase 4 结束 | ≥ 95% | hitool-poi 占位 + hitool-ai provider |
| Phase 5 结束 | ≥ 99% | 命名收尾 + facade 补全 |
| Phase 6 结束 | **100%** | **每个 hutool .java 都有 hitool-rs 对应** |
| v1.0.0 | **100%（除 hitool-poi）** | 锁定覆盖率 |
...
**对「hitool 独有扩展」（无 Java 对应）的处理**：

hitool-rs 可能新增 hutool Java 没有的模块（如 `advanced_codec.rs`、`murmur3` 等）。这些**不受 1:1 要求约束**，允许自由扩展。
...
| hutool 包 | hitool 子目录 | 文件数对比 | 迁移完成度 |
|---|---|---:|---:|
...
**总体 hitool-core 命名一致性 ≈ 90%**，但有以下"缺失 facade 类"待补：

| 用户期待类名 | hitool-rs 实际 | 计划动作 |
|---|---|---|
...
| `SecureUtil` | ❌ 不存在 | Phase 1.3 新建 `util/secure_util.rs`（re-export `hitool-crypto::*`） |
| `DigestUtil` | ❌ 不存在 | Phase 1.3 新建 `util/digest_util.rs`（re-export `hitool-crypto::digest::*`） |
...
| `DES` / `AES` / `RSA` | 移至 `hitool-crypto` | Phase 1.4 加 re-export 兼容 |

---

## 六、核心类型/Trait 签名（重点 hitool-core）
...
```rust
// hitool-core/src/collection/coll_util.rs
...
```rust
// hitool-core/src/date/date_util.rs

/// 1:1 对齐 `cn.hutool.core.date.DateUtil`
pub struct DateUtil;

/// DateTime 类型别名（hitool-rs 的简化）
...
```rust
// hitool-core/src/io/file_util.rs
...
```rust
// hitool-core/src/util/number_util.rs
...
```rust
// hitool-core/src/str_util.rs （需扩充）
...
```rust
// hitool-core/src/map/map_util.rs
...
### 6.7 `IdUtil` / `IdcardUtil` / `SecureUtil`（hitool-core 内直接实现）

```rust
// hitool-core/src/util/id_util.rs
...
```rust
// hitool-core/src/util/secure_util.rs （新增 facade，委托到 hitool-crypto）
//! 1:1 对齐 `cn.hutool.crypto.SecureUtil`
pub struct SecureUtil;

impl SecureUtil {
    pub fn md5(data: &str) -> String {
        hitool_crypto::digest::md5_hex(data)
    }
    pub fn sha1(data: &str) -> String {
        hitool_crypto::digest::sha1_hex(data)
    }
    pub fn sha256(data: &str) -> String {
        hitool_crypto::digest::sha256_hex(data)
    }
    pub fn sha512(data: &str) -> String {
        hitool_crypto::digest::sha512_hex(data)
    }
...
### 7.1 当前状态：`hitool-macros` 仅有 1 个文件

```text
crates/hitool-macros/
├── Cargo.toml
└── src/
...
> 所以 hitool-macros 主要服务于**编译期派生宏**，而非运行时注解处理器。

### 7.3 已有宏（保留）

```rust
// hitool-macros/src/lib.rs
...
| `Page` / `PageResult` | 🟡 在 hitool-core 已有 `page_util.rs` | Phase 2 整合 |
...
### 8.4 hitool-extra（基于 qrcode / zip / etc）
...
| `validation/*` (BeanValidation) | 🟡 `validator.rs` 在 hitool-core | Phase 2 |
...
| 异步模型 | **核心 sync + 适配层 async** | 借鉴 sa-token-rs；hitool-core 无 IO，sync 即可 |
| 全局状态 | `OnceLock<Arc<T>>` | 借鉴 sa-token-rs |
| 错误处理 | **每 crate 一个根 enum + thiserror** | 借鉴 sa-token-rs；hitool-core 已有 11+ 子 enum，避免巨型 enum |
| 序列化 | `serde` + `serde_json::Value` | Rust 生态标准 |
| JSON | `serde_json`（hitool-json facade） | Hutool JSONUtil 风格 facade + serde 底层 |
| HTTP | `reqwest` + `tower`（可选） | Rust 生态标准 |
| DB | `sqlx`（**不造 ORM**） | 借鉴 sa-token-rs "不包装 ORM" 原则 |
| 缓存 | `moka` | 高性能进程内缓存 |
| 定时调度 | `tokio-cron-scheduler`（业务侧） + 自研 cron 解析（hitool-cron） | 双轨：自研 cron 表达式 + 借用 tokio 调度 |
...
> 对 hitool-rs 当前依赖和架构进行**事实核对 + 提升建议**。

#### 9.1.1 实际依赖现状（基于 hitool-rs 工作区 Cargo.toml 实测）

| crate | 直接依赖（`=` 行） | optional 依赖 | 实际可用特性 |
|---|---:|---:|---|
| `hitool` (facade) | 63 | 20 | 全部通过 `dep:hitool-*` 特性聚合 |
| `hitool-extra` | 17 | 7 | `default = ["archive", "qrcode", "emoji", "pinyin"]` |
| `hitool-core` | 15 | 6 | `default = []`；`swing`/`img`/`async` 三个 feature |
| `hitool-http` | 11 | 4 | `default = ["useragent", "html"]`；`blocking` opt-in |
| `hitool-db` | 9 | 2 | `default = ["sqlite"]`；`postgres`/`mysql` opt-in |
| `hitool-crypto` | 5 | 0 | `default = []`；`legacy` opt-in |
| `hitool-captcha` | 9 | 3 | `default = ["raster"]`；`audio` opt-in |
| `hitool-json` | 3 | 0 | `default = []` |
| `hitool-cache` / `hitool-cron` / `hitool-script` / `hitool-setting` / `hitool-system` / `hitool-jwt` / `hitool-log` / `hitool-aop` / `hitool-bloom-filter` / `hitool-dfa` / `hitool-socket` / `hitool-ai` / `hitool-macros` / `hitool-compat-hutool` | 3~5 | 0 | 均为 `default = []` |
...
| DDD4J Java 领域 | Rust 生态 | hitool-rs 状态 |
|---|---|---|
| JSON：Jackson / Gson / FastJSON | `serde_json` | ✅ `hitool-json` |
| YAML：SnakeYAML | `serde_yaml_ng` | ⚠️ 仅 workspace.dependencies 中定义，**hitool-setting 未启用** |
| HTTP：OkHttp / Apache HttpClient | `reqwest` | ✅ `hitool-http` |
| 加密：BouncyCastle | `aes` / `sha2` / `sm2` / `sm3` / `sm4` / `rsa` | ✅ `hitool-crypto`（30+ 算法实现） |
| 数据库连接池：HikariCP / Druid | `sqlx::Pool` | ✅ `hitool-db` |
| 缓存：Caffeine / Ehcache | `moka` | ✅ `hitool-cache` |
| 邮件：javax.mail | `lettre` | ✅ `hitool-extra::mail`（feature-gated） |
| 定时任务：Quartz | `cron` | ✅ `hitool-cron`（自研解析）+ workspace 引入 `cron` crate |
| 脚本：JSR-223 | `rhai` | ✅ `hitool-script` |
| 日志：Logback / SLF4J | `tracing` + `tracing-subscriber` | ✅ `hitool-log` |
| 并发：JDK concurrent | `tokio` + `parking_lot` | ✅ `hitool-core::async` (feature-gated) |
| 文件监控：Commons IO | `notify` | ✅ workspace.dependencies，但**未在任何 crate 启用** |
| 图片：Java ImageIO | `image` | ✅ `hitool-core::img` + `hitool-extra::image`（feature-gated） |
| QR 码：zxing | `qrcode`（svg feature） | ✅ `hitool-extra::qrcode` |
| UUID：java.util.UUID | `uuid`（v3/v4/v5） | ✅ `hitool-core` |
| CSV：Commons CSV | `csv` | ✅ `hitool-core` |
| Unicode：ICU4J | `unicode-general-category` + `unicode-normalization` | ✅ `hitool-core` |
| 配置：Commons Config | `config` | ✅ `hitool-setting` |
| 密钥安全：Java Crypto | `secrecy` + `zeroize` | ✅ `hitool-crypto` |
| Excel：POI / EasyExcel | `rust_xlsxwriter` + `quick-xml` + `zip` | ✅ workspace.dependencies；**`hitool-poi` crate 待建（Phase 3.1 占位）** |
| 脚本测试：JUnit | `proptest` | ✅ workspace.dependencies（dev-dependencies） |
| 正则：JDK Pattern | `regex` + `fancy-regex` + `aho-corasick` | ✅ `hitool-core` / `hitool-dfa` / `hitool-http::html` |
| 编码：Commons Codec | `base64` + `hex` + `data-encoding` + `encoding_rs` | ✅ `hitool-core` |
| 数据库迁移：Flyway | `sqlx::migrate!`（内置） | ✅ `hitool-db` 推荐用法 |

#### 9.1.3 建议补全（**按 DDD4J 分析 + hitool-rs 现状综合评估**）

| 优先级 | 补全项 | Rust crate | 目标 crate | 评估依据 |
|---|---|---|---|---|
| 🟢 高 | 启用 `serde_yaml_ng` 到 `hitool-setting` | `serde_yaml_ng` | `hitool-setting` | workspace 已声明但未启用；hutool-setting 明确支持 YAML |
| 🟢 高 | 启用 `notify` 到 hitool-core 的 Watch 模块 | `notify` | `hitool-core::io::watch` | 对齐 `cn.hutool.core.io.watch.WatchMonitor`（已实现），但缺底层依赖 |
| 🟢 高 | `itertools` 加入 hitool-core | `itertools` | `hitool-core` | Rust 生态集合迭代标准补充；hutool-core 大量组合操作可受益 |
| 🟡 中 | `num` / `statrs` 加入 hitool-core | `num` + `statrs` | `hitool-core` | 对齐 `cn.hutool.core.math.*` 缺失的统计/数值工具 |
| 🟡 中 | `eventsource-stream` 加入 hitool-ai | `eventsource-stream` | `hitool-ai` | DDD4J 推荐；当前 hitool-ai 是否已用待确认 |
| 🟡 中 | 新建 `crates/hitool-cache-redis`（DAO 扩展） | `redis` | **新 crate** | 对齐 `hutool-cache` 的 Redis 实现（RedisCache） |
| 🟡 中 | `tracing-appender` + `tracing-json` 增强 hitool-log | `tracing-appender` + `tracing-json` | `hitool-log` | DDD4J 推荐；当前 `tracing-subscriber` 已够用，json 输出为增强 |
| 🟠 低 | 新建 `crates/hitool-mq`（Kafka/RabbitMQ/MQTT） | `rdkafka` / `lapin` / `rumqttc` | **新 crate** | DDD4J 推荐；hutool 无原生 MQ，需自定边界 |
| 🟠 低 | 新建 `crates/hitool-distributed`（Nacos/Redis/Dubbo） | `nacos-rust` / `redis` | **新 crate** | DDD4J 推荐；hutool-extra 中有 Dubbo/Nacos 适配 |
| 🟠 低 | 新建 `crates/hitool-security`（axum-login / tower-sessions） | `axum-login` + `tower-sessions` | **新 crate** | DDD4J 推荐；hutool-security 对标 Shiro/Sa-Token |

#### 9.1.4 ❌ 不建议补全（**与项目原则冲突**）

| 建议项 | 原因 |
|---|---|
| ❌ 新增 `sea-orm` / `diesel` ORM | 违反原则 7「不造 ORM」、hitool 架构决策 §9「sqlx 直接构建」；ORM 应作为上层应用框架职责 |
| ❌ 在 `hitool-http` 中加 `axum` Web Server | hitool-http 是**客户端**，Server 模块应新建 `hitool-http-server` 独立 crate（参考 sa-token-rs 的 starter 拆分） |
...
| 编译时长 | 单 crate 增量编译 ≤ 10s | `cargo build -p hitool-core --timings` |
| 默认特性膨胀 | `default = []` 优先 | `cargo metadata --format-version 1` |
| 特性互斥 | `legacy` 之类 opt-in 互斥 | CI 矩阵编译 |

#### 9.1.6 feature 设计原则（**新增**）

参考 DDD4J 中关于「hutool-all vs 单 crate」的设计决策，hitool-rs 的 feature 设计遵守：

```toml
# crates/hitool/Cargo.toml
...
# 单 crate 必须为 opt-in
crypto-legacy = ["crypto", "hitool-crypto/legacy"]   # 显式启用
http-blocking = ["http", "hitool-http/blocking"]      # 阻塞 IO 显式启用
db-postgres = ["db", "hitool-db/postgres"]            # 驱动显式选择
...
**核心规则**：
1. **默认零**：`default = []`（除 facade `hitool` 默认 `core` + `json`）
...
```text
v0.1.x (Phase 1) ─── hitool-core 命名收尾 + **hitool-extra 优先补全**（缺 170 文件，最高优先级）
    ├── Phase 1.1: hitool-extra 补 compress.archiver/extractor（七牛/腾讯/阿里/华为/京东 OSS 适配）
    ├── Phase 1.2: hitool-extra 补 mail（Mail/MailAccount/MailUtil）
    ├── Phase 1.3: hitool-extra 补 expression 引擎（Aviator/JEXL/JfireEL/Mvel/QLExpress/Rhino/SpEL）
    ├── Phase 1.4: hitool-extra 补 template 引擎（Beetl/Enjoy/Freemarker/Jetbrick/Rythm/Thymeleaf/Velocity/Wit）
    └── Phase 1.5: hitool-extra 补 ftp/ssh（Ftp/AbstractFtp/Sftp/JschUtil 等）

v0.2.x (Phase 2) ─── hitool-cron + hitool-db 核心补全
    ├── Phase 2.1: hitool-cron 补 41 文件（pattern 子包 + TaskExecutor + Listener + TimingWheel）
    ├── Phase 2.2: hitool-db 补 75 文件（Entity/Page/Query/SqlBuilder + Druid/Hikari DSFactory + 9 方言）
    ├── Phase 2.3: hitool-db 补 handler/meta/nosql 子包
    └── Phase 2.4: hitool-extra 补 tokenizer（8 engine：ansj/hanlp/ikanalyzer/jcseg/jieba/mmseg/mynlp/word）

v0.3.x (Phase 3) ─── hitool-crypto + hitool-http 补全
    ├── Phase 3.1: hitool-crypto 补 48 文件（SymmetricCrypto/SymmetricAlgorithm/RSA/ECIES/SM2/SM3/SM4/DES/RC4/HMac）
    ├── Phase 3.2: hitool-http 补 47 文件（HttpUtil 静态 facade + Server 子包 + SSL 子包 + Soap 子包）
    ├── Phase 3.3: hitool-aop + hitool-cache + hitool-bloom-filter + hitool-captcha 补全
    └── Phase 3.4: hitool-json + hitool-jwt + hitool-script + hitool-setting + hitool-system + hitool-socket 补全

v0.4.x (Phase 4) ─── hitool-poi 占位骨架 + hitool-ai 7 provider
    ├── Phase 4.1: 创建 crates/hitool-poi + 78 个 .rs 占位文件
    ├── Phase 4.2: 为占位文件添加 rustdoc "原 Java 文件/方法" 注释
    ├── Phase 4.3: hitool-ai 补 DeepSeek/Doubao/Gemini/Grok/Hutool/Ollama
    └── Phase 4.4: hitool-extra 补 emoji/pinyin/spring/servlet/validation

v0.5.x (Phase 5) ─── hitool-core 命名收尾（双重路径收尾 + facade 补全）
    ├── Phase 5.1: 解决 hitool-core 双重路径问题（保留全部，pub use 重新导出）
    ├── Phase 5.2: 补全缺失 facade 类（SetUtil/URLEncodeUtil/URLDecodeUtil/RegexUtil）
    ├── Phase 5.3: SecureUtil/DigestUtil/Base32Util/Base64Util 在 hitool-core 直接实现
...
| ⚪ 冻结 | **hitool-compat-hutool** | 0 | 1（114 行） | — | — | 冻结维持现状 |
| ✅ | **hutool-core** | 713 | 780 | -67（多 67） | 109% | Phase 5 收尾 |

**关键修正**：
1. **hitool-extra 缺 170 文件，是绝对的 P0 优先级**（最高）
2. **hitool-db 缺 75 文件**，必须先迁移（用户硬性要求）
3. **hitool-cron / hitool-http / hitool-crypto** 都是 P0 严重不足
4. **hitool-compat-hutool 冻结**（114 行无迁移价值）
...
| Phase 1 | 0.1.x | **hitool-extra 优先补全**（缺 170 文件）+ 依赖治理 | 启用 `serde_yaml_ng` / `notify` / `itertools` |
| Phase 2 | 0.2.x | cron/db/extra 核心补全 | 启用 `num` / `statrs` |
| Phase 3 | 0.3.x | hitool-poi 占位 + ai 7 provider | 启用 `eventsource-stream` |
...
| **Phase 7** | **0.6~0.8** | **依赖与架构提升（新增）** | **新建 hitool-cache-redis / hitool-mq / hitool-distributed** |
...
| 改动 | 当前 | 目标 |
|---|---|---|
| `hitool-setting` 加入 `serde_yaml_ng` | ❌ 未启用 | ✅ 启用（hutool-setting 原生支持 YAML） |
| `hitool-core` 启用 `notify` | ❌ 未启用 | ✅ 在 `io::watch` 模块启用 |
| `hitool-core` 加入 `itertools` | ❌ 缺失 | ✅ 新增（Rust 集合迭代标准） |
| `hitool-core` 加入 `num` + `statrs` | ❌ 缺失 | ✅ 新增（数学/统计） |

**验收标准**：
```rust
// hitool-setting/src/yaml.rs
use serde_yaml_ng;
let cfg: MyConfig = serde_yaml_ng::from_str(text)?;

// hitool-core/src/io/watch.rs（已有 WatchMonitor）
use notify::{RecommendedWatcher, RecursiveMode};
let mut watcher = notify::recommended_watcher(|res| { ... })?;
watcher.watch(Path::new("."), RecursiveMode::Recursive)?;
```

#### **Phase 7.2：新建 hitool-cache-redis（DAO 扩展，1 周）**

**目标**：补齐 hutool-cache 缺失的 Redis 实现。

```toml
# crates/hitool-cache-redis/Cargo.toml
[dependencies]
hitool-core.workspace = true
...
**Rust 实现**：
```rust
// crates/hitool-cache-redis/src/lib.rs
...
#### **Phase 7.3：新建 hitool-ai SSE 增强（1 周）**

**目标**：把 `eventsource-stream` 加入 hitool-ai，标准化流式响应。

```toml
# crates/hitool-ai/Cargo.toml
[dependencies]
# 现有 + 新增
...
#### **Phase 7.4：新建 hitool-mq（消息队列，低优先级 / 3~4 周）**
...
#### **Phase 7.5：新建 hitool-distributed（分布式中间件，低优先级 / 3~4 周）**

**决策**：**作为 opt-in 扩展**，默认不引入。

```text
crates/hitool-distributed/
...
#### **Phase 7 不启动的项（DDD4J 推荐但与 hitool-rs 原则冲突）**

| 项 | 不启动原因 |
|---|---|
| ❌ `sea-orm` / `diesel` ORM | 违反「不造 ORM」原则 |
| ❌ `hitool-http` 内嵌 `axum` Server | 应新建 `hitool-http-server` 独立 crate |
...
### **Phase 1.2：hitool-core 目录结构重组（解决双重路径 + 平铺问题）**（2~3 周，**P0**）
...
| 指标 | hutool Java | hitool-rs Rust（当前） | 评价 |
|---|---|---|---|
...
**目标**：让 hitool-core 严格按 hutool Java 的 24 子包组织，每个子包是"目录索引"，顶层 lib.rs 只做 `mod` 声明和统一 re-export。
...
```bash
# Step 1: 创建新位置目录
mkdir -p crates/hitool-core/src/text  # 已有，待确认内容
mkdir -p crates/hitool-core/src/util  # 已有，待确认内容

# Step 2: 用 git mv 把顶层 .rs 移动到对应子目录（★ "不删减"用 git mv 保留历史）
cd crates/hitool-core/src
...
```rust
// hitool-core/src/util/secure_util.rs （新增）
//! 1:1 对齐 `cn.hutool.crypto.SecureUtil`
//! 实现：纯委托到 `hitool_crypto` crate

pub struct SecureUtil;
impl SecureUtil {
    pub fn md5(s: &str) -> String { hitool_crypto::digest::md5_hex(s) }
    pub fn sha1(s: &str) -> String { hitool_crypto::digest::sha1_hex(s) }
    pub fn sha256(s: &str) -> String { hitool_crypto::digest::sha256_hex(s) }
    pub fn sha512(s: &str) -> String { hitool_crypto::digest::sha512_hex(s) }
...
```rust
// hitool-core/src/util/digest_util.rs （新增）
...
### **Phase 2.1：hitool-cron 补全**（2~3 周）
...
mkdir -p crates/hitool-poi/src/{excel/{cell/{setters,values},editors,reader,sax/handler,style},ofd,word,exceptions}
mkdir -p crates/hitool-poi/examples
mkdir -p crates/hitool-poi/tests

# 2. 在 Cargo.toml 中添加
echo 'hitool-poi = { path = "crates/hitool-poi" }' >> crates/hitool/Cargo.toml
...
```rust
// crates/hitool-poi/src/excel/excel_util.rs
...
### **Phase 3.3：hitool-ai 补全**（3~4 周）

每个 provider 5 文件：

```text
hitool-ai/src/provider/
├── openai.rs                           # ✅ 已实现
...
### 11.2 hitool-core 依赖（保持现状，不引入 async）
...
```rust
// hitool-core/src/str_util.rs（同文件 unit test）
#[cfg(test)]
mod tests {
    use super::*;
...
└── migration/                            # 迁移审计文档
    ...
    ├── rust-tree-full.md                 # ★ 新增：hitool-rs 完整目录树
...
| hitool-core 双重路径问题 | 不删减，pub use 重新导出，保留全部实现 |
| 缺失 facade 类（SetUtil/URLDecodeUtil 等） | Phase 1.3 集中补全 |
| hitool-poi 占位骨架 vs easyexcel-rs 依赖 | 仅做占位，等待 easyexcel-rs 等完成 |
| Rust 与 Java 语义差异大（如 `Collection<Object>` → 强类型） | 类型映射表固定 + serde_json::Value 兜底 |
| Java 反射注解 → Rust 宏心智模型差异 | hitool-macros 提供 derive + helper attribute |
...
### 15.1 hutool-core ↔ hitool-core（核心模块）

| Java 源文件 | Rust 源文件 | Phase |
|---|---|---|
| `cn/hutool/core/util/StrUtil.java` | `hitool-core/src/str_util.rs` + `text/str_*.rs` | 1.1 |
| `cn/hutool/core/util/ArrayUtil.java` | `hitool-core/src/array_util.rs` | 1 |
| `cn/hutool/core/util/PrimitiveArrayUtil.java` | `hitool-core/src/primitive_array_util.rs` | 1 |
| `cn/hutool/core/util/BooleanUtil.java` | `hitool-core/src/boolean_util.rs` | 1 |
| `cn/hutool/core/util/ByteUtil.java` | `hitool-core/src/byte_util.rs` | 1 |
| `cn/hutool/core/util/CharUtil.java` | `hitool-core/src/char_util.rs` | 1 |
| `cn/hutool/core/util/CharsetUtil.java` | `hitool-core/src/charset_util.rs` | 1 |
| `cn/hutool/core/util/ClassLoaderUtil.java` | `hitool-core/src/class_loader_util.rs` | 1 |
| `cn/hutool/core/util/ClassUtil.java` | `hitool-core/src/class_util.rs` | 1 |
| `cn/hutool/core/util/CoordinateUtil.java` | `hitool-core/src/coordinate_util.rs` | 1 |
| `cn/hutool/core/util/CreditCodeUtil.java` | `hitool-core/src/credit_code_util.rs` | 1 |
| `cn/hutool/core/util/DesensitizedUtil.java` | `hitool-core/src/desensitized_util.rs` | 1 |
| `cn/hutool/core/util/EnumUtil.java` | `hitool-core/src/enum_util.rs` | 1 |
| `cn/hutool/core/util/EscapeUtil.java` | `hitool-core/src/escape_util.rs` | 1 |
| `cn/hutool/core/util/HashUtil.java` | `hitool-core/src/hash_util.rs` | 1 |
| `cn/hutool/core/util/HexUtil.java` | `hitool-core/src/hex_util.rs` | 1 |
| `cn/hutool/core/util/IdUtil.java` | `hitool-core/src/id.rs` + `src/util/id_util.rs` | 1 |
| `cn/hutool/core/util/IdcardUtil.java` | `hitool-core/src/idcard_util.rs` | 1 |
| `cn/hutool/core/util/JAXBUtil.java` | `hitool-core/src/jaxb_util.rs` | 2 |
| `cn/hutool/core/util/JNDIUtil.java` | （跳过，`unsafe-to-copy`） | — |
| `cn/hutool/core/util/JdkUtil.java` | （跳过，无 Rust 对应） | — |
| `cn/hutool/core/util/ModifierUtil.java` | `hitool-core/src/modifier_util.rs` | 3 |
| `cn/hutool/core/util/NumberUtil.java` | `hitool-core/src/number_util.rs` | 1 |
| `cn/hutool/core/util/ObjUtil.java` | `hitool-core/src/object_util.rs` | 1 |
| `cn/hutool/core/util/ObjectUtil.java` | `hitool-core/src/object_util.rs` | 1 |
| `cn/hutool/core/util/PageUtil.java` | `hitool-core/src/page_util.rs` | 1 |
| `cn/hutool/core/util/PhoneUtil.java` | `hitool-core/src/phone_util.rs` | 1 |
| `cn/hutool/core/util/RadixUtil.java` | `hitool-core/src/radix_util.rs` | 1 |
| `cn/hutool/core/util/RandomUtil.java` | `hitool-core/src/random_util.rs` | 1 |
| `cn/hutool/core/util/ReUtil.java` | `hitool-core/src/re_util.rs` + `src/util/regex_util.rs`（新） | 1.3 |
| `cn/hutool/core/util/ReferenceUtil.java` | `hitool-core/src/reference_util.rs` | 1 |
| `cn/hutool/core/util/ReflectUtil.java` | `hitool-core/src/reflect_util.rs`（简化） | 2 |
| `cn/hutool/core/util/RuntimeUtil.java` | `hitool-core/src/runtime_util.rs` | 1 |
| `cn/hutool/core/util/SerializeUtil.java` | `hitool-core/src/util/serialize_util.rs`（新） | 1.3 |
| `cn/hutool/core/util/ServiceLoaderUtil.java` | `hitool-core/src/util/service_loader_util.rs`（新） | 1.3 |
| `cn/hutool/core/util/SystemPropsUtil.java` | （合并到 `system_props_util.rs`） | 1 |
| `cn/hutool/core/util/TypeUtil.java` | `hitool-core/src/type_util.rs` | 1 |
| `cn/hutool/core/util/URLUtil.java` | `hitool-core/src/url_util.rs` + `src/net/url_*.rs` | 1 |
| `cn/hutool/core/util/VersionUtil.java` | `hitool-core/src/version_util.rs` | 1 |
| `cn/hutool/core/util/XmlUtil.java` | `hitool-core/src/xml_util.rs` | 1 |
| `cn/hutool/core/util/ZipUtil.java` | `hitool-core/src/zip_util.rs` | 1 |
| **`cn/hutool/core/util/SetUtil.java`** | ❌ → 新建 `hitool-core/src/util/set_util.rs` | **1.3** |
| **`cn/hutool/core/util/URLEncodeUtil.java`** | ❌ → 新建 `hitool-core/src/net/url_encode_util.rs` | **1.3** |
| **`cn/hutool/core/util/URLDecodeUtil.java`** | ❌ → 新建 `hitool-core/src/net/url_decode_util.rs` | **1.3** |
| `cn/hutool/core/io/IoUtil.java` | `hitool-core/src/io_util.rs` + `src/io/io_util.rs` | 1 |
| `cn/hutool/core/io/FileUtil.java` | `hitool-core/src/file_util.rs` + `src/io/file/file_util.rs` | 1 |
| `cn/hutool/core/io/ResourceUtil.java` | `hitool-core/src/io/resource/resource_util.rs` | 1 |
| `cn/hutool/core/collection/CollUtil.java` | `hitool-core/src/coll_util.rs` + `src/collection/coll_util.rs` | 1 |
| `cn/hutool/core/collection/CollectionUtil.java` | `hitool-core/src/collection/collection_util.rs` | 1 |
| `cn/hutool/core/collection/ListUtil.java` | `hitool-core/src/list_util.rs` | 1 |
| `cn/hutool/core/collection/IterUtil.java` | `hitool-core/src/iter_util.rs` | 1 |
| `cn/hutool/core/collection/CollStreamUtil.java` | `hitool-core/src/coll_stream_util.rs` | 1 |
| `cn/hutool/core/map/MapUtil.java` | `hitool-core/src/map_util.rs` + `src/map/map_util.rs` | 1 |
| `cn/hutool/core/date/DateUtil.java` | `hitool-core/src/date/date_util.rs` | 1 |
| `cn/hutool/core/date/DateTime.java` | `hitool-core/src/date/date_time.rs` + type alias `DateTime` | 1 |
| `cn/hutool/core/date/LocalDateTimeUtil.java` | `hitool-core/src/date/local_date_time_util.rs` | 1 |
| `cn/hutool/core/bean/BeanUtil.java` | `hitool-core/src/bean_util.rs` + `src/bean/bean_util.rs` | 2 |
| **`cn/hutool/crypto/SecureUtil.java`** | ❌ → 新建 `hitool-core/src/util/secure_util.rs`（委托 facade） | **1.4** |
| **`cn/hutool/crypto/digest/DigestUtil.java`** | ❌ → 新建 `hitool-core/src/util/digest_util.rs`（委托 facade） | **1.4** |

### 15.2 hutool-cron ↔ hitool-cron（核心模块）

| Java 源文件 | Rust 源文件 | Phase |
|---|---|---|
| `cn/hutool/cron/CronUtil.java` | `hitool-cron/src/lib.rs` (facade) | 2.1 |
| `cn/hutool/cron/CronPattern.java` | `hitool-cron/src/expression.rs` + `cron_pattern.rs` | 2.1 |
| `cn/hutool/cron/CronPatternBuilder.java` | `hitool-cron/src/cron_pattern_builder.rs` | 2.1 |
| `cn/hutool/cron/CronPatternUtil.java` | `hitool-cron/src/parser.rs` + `cron_pattern_util.rs` | 2.1 |
| `cn/hutool/cron/Scheduler.java` | `hitool-cron/src/scheduler.rs` | 2.1 |
| `cn/hutool/cron/CronTask.java` | `hitool-cron/src/cron_task.rs` | 2.1 |
| `cn/hutool/cron/CronConfig.java` | `hitool-cron/src/cron_config.rs` | 2.1 |
| `cn/hutool/cron/CronException.java` | `hitool-cron/src/cron_exception.rs` | 2.1 |
| `cn/hutool/cron/CronTimer.java` | `hitool-cron/src/cron_timer.rs` | 2.1 |
| `cn/hutool/cron/SystemTimer.java` | `hitool-cron/src/system_timer.rs` | 2.1 |
| `cn/hutool/cron/Task.java` | `hitool-cron/src/task.rs` | 2.1 |
| `cn/hutool/cron/TaskExecutor.java` | `hitool-cron/src/task_executor.rs` | 2.1 |
| `cn/hutool/cron/TaskExecutorManager.java` | `hitool-cron/src/task_executor_manager.rs` | 2.1 |
| `cn/hutool/cron/TaskLauncher.java` | `hitool-cron/src/task_launcher.rs` | 2.1 |
| `cn/hutool/cron/TaskLauncherManager.java` | `hitool-cron/src/task_launcher_manager.rs` | 2.1 |
| `cn/hutool/cron/TaskListener.java` | `hitool-cron/src/task_listener.rs` | 2.1 |
| `cn/hutool/cron/TaskListenerManager.java` | `hitool-cron/src/task_listener_manager.rs` | 2.1 |
| `cn/hutool/cron/TaskTable.java` | `hitool-cron/src/task_table.rs` | 2.1 |
| `cn/hutool/cron/TimerTask.java` | `hitool-cron/src/timer_task.rs` | 2.1 |
| `cn/hutool/cron/TimerTaskList.java` | `hitool-cron/src/timer_task_list.rs` | 2.1 |
| `cn/hutool/cron/listener/SimpleTaskListener.java` | `hitool-cron/src/listener/simple_task_listener.rs` | 2.1 |
| `cn/hutool/cron/pattern/*` (5 文件) | `hitool-cron/src/pattern/*.rs` | 2.1 |
| `cn/hutool/cron/pattern/matcher/*` (4 文件) | `hitool-cron/src/pattern/matcher/*.rs` | 2.1 |
| `cn/hutool/cron/task/InvokeTask.java` | `hitool-cron/src/task/invoke_task.rs` | 2.1 |
| `cn/hutool/cron/task/RunnableTask.java` | `hitool-cron/src/task/runnable_task.rs` | 2.1 |
| `cn/hutool/cron/timingwheel/TimingWheel.java` | `hitool-cron/src/timingwheel/timing_wheel.rs` | 2.1 |

### 15.3 hutool-poi ↔ hitool-poi（**全部为占位**）

| Java 源文件 | Rust 源文件 | Phase |
|---|---|---|
| `cn/hutool/poi/excel/ExcelUtil.java` | `crates/hitool-poi/src/excel/excel_util.rs`（占位） | 3.1 |
| `cn/hutool/poi/excel/ExcelReader.java` | `crates/hitool-poi/src/excel/excel_reader.rs`（占位） | 3.1 |
| `cn/hutool/poi/excel/ExcelWriter.java` | `crates/hitool-poi/src/excel/excel_writer.rs`（占位） | 3.1 |
| `cn/hutool/poi/excel/BigExcelWriter.java` | `crates/hitool-poi/src/excel/big_excel_writer.rs`（占位） | 3.1 |
| `cn/hututl/poi/excel/RowUtil.java` | `crates/hitool-poi/src/excel/row_util.rs`（占位） | 3.1 |
| ...（其余 73 个 .java 全部对应 73 个 .rs 占位）| | 3.1 |
| `cn/hutool/poi/word/Word07Writer.java` | `crates/hitool-poi/src/word/word07_writer.rs`（占位） | 3.1 |
| `cn/hutool/poi/word/WordUtil.java` | `crates/hitool-poi/src/word/word_util.rs`（占位） | 3.1 |
| `cn/hutool/poi/ofd/OfdWriter.java` | `crates/hitool-poi/src/ofd/ofd_writer.rs`（占位） | 3.1 |
| `cn/hutool/poi/exceptions/POIException.java` | `crates/hitool-poi/src/exceptions/poi_exception.rs`（占位） | 3.1 |

### 15.4 hutool-ai ↔ hitool-ai

| Java 源文件 | Rust 源文件 | Phase |
|---|---|---|
| `cn/hutool/ai/AIUtil.java` | `crates/hitool-ai/src/lib.rs` (facade) | 1 |
| `cn/hutool/ai/core/AIService.java` | `crates/hitool-ai/src/client.rs` | 1 |
| `cn/hutool/ai/core/AIConfig.java` | `crates/hitool-ai/src/request.rs` (struct) | 1 |
| `cn/hutool/ai/core/Message.java` | `crates/hitool-ai/src/message.rs` | 1 |
| `cn/hutool/ai/Models.java` | `crates/hitool-ai/src/model.rs` | 1 |
| `cn/hutool/ai/model/openai/*.java` | `crates/hitool-ai/src/provider/openai.rs` | 1 |
| `cn/hutool/ai/model/deepseek/*.java` | `crates/hitool-ai/src/provider/deepseek.rs`（新） | 3.3 |
| `cn/hutool/ai/model/doubao/*.java` | `crates/hitool-ai/src/provider/doubao.rs`（新） | 3.3 |
| `cn/hutool/ai/model/gemini/*.java` | `crates/hitool-ai/src/provider/gemini.rs`（新） | 3.3 |
| `cn/hutool/ai/model/grok/*.java` | `crates/hitool-ai/src/provider/grok.rs`（新） | 3.3 |
| `cn/hutool/ai/model/hutool/*.java` | `crates/hitool-ai/src/provider/hutool.rs`（新） | 3.3 |
| `cn/hutool/ai/model/ollama/*.java` | `crates/hitool-ai/src/provider/ollama.rs`（新） | 3.3 |
...
> 用户硬性要求：「hutool 中的除了 hutool-poi，每一个模块，每一个文件在 hitool-rs 中必须有对应的实现」
> 因此第一步必须建立可验收的自动化脚本。
...
#!/usr/bin/env bash
# 验证 hutool Java 文件 ↔ hitool-rs Rust 文件 1:1 覆盖率（除 hutool-poi 外）
set -e
HUTOOL_ROOT="/Users/wandl/workspaces/workspace-github/hutool"
HITOOL_ROOT="/Users/wandl/workspaces/workspace-github/hitool-rs"
PHASE="${1:-1}"
...
# 2. 列出 hitool-rs 除 hitool-poi 外全部 .rs
find "$HITOOL_ROOT/crates" -name "*.rs" \
  -not -path "*/target/*" -not -path "*/tests/*" -not -path "*/examples/*" \
  -not -path "*/hitool-poi/*" \
  | sed 's|.*/||' | sed 's|.rs$||' \
  | sort -u > /tmp/hitool_files.txt
...
while IFS= read -r java_path; do
    ...
    if grep -q "^${snake}$" /tmp/hitool_files.txt; then
        covered=$((covered + 1))
    fi
...
```bash
mkdir -p /Users/wandl/workspaces/workspace-github/hitool-rs/docs/migration
# 创建以下文档：
touch docs/GUIDE.md
...
### Step 2：创建 hitool-poi 占位骨架（1~2 天）

```bash
mkdir -p crates/hitool-poi/src/{excel,ofd,word,exceptions}
mkdir -p crates/hitool-poi/examples
mkdir -p crates/hitool-poi/tests
...
for java_file in $(find /Users/wandl/workspaces/workspace-github/hutool/hutool-poi/src/main/java -name "*.java" | sed 's|.*/cn/hutool/poi/||' | sed 's|.java$|.rs|'); do
    target="crates/hitool-poi/src/$java_file"
...
### Step 3：在 workspace 中注册 hitool-poi（5 分钟）

```toml
# Cargo.toml
[workspace]
members = ["crates/*"]
# hitool-poi 已在 crates/ 下，自动包含
```

```toml
# crates/hitool/Cargo.toml
[dependencies]
hitool-poi = { path = "../hitool-poi", optional = true }
...
### Step 7：补充 Phase 1.2 hitool-core 双重路径（1 天）
...
| 差异点 | Hutool Java 做法 | HiTool-Rs 做法 |
|---|---|---|
...
- **easyexcel-rs**：`/Users/wandl/workspaces/workspace-github/easyexcel-rs`（hitool-poi 实现参考）
...
## 附录 C：Hutool v5.8.x 模块 → HiTool crate 映射表

| Hutool 模块 | HiTool crate | 迁移完成度 | 计划 Phase |
|---|---|---:|---|
| `hutool-bom` | workspace | 100% | 0 |
| `hutool-all` | `hitool` (facade) | 100% | 0 |
| `hutool-core` | `hitool-core` | **80%** | 1.x |
| `hutool-aop` | `hitool-aop` | 50% | 3 |
| `hutool-bloomFilter` | `hitool-bloom-filter` | 25% | 4 |
| `hutool-cache` | `hitool-cache` | 30% | 4 |
| `hutool-captcha` | `hitool-captcha` | 50% | 4 |
| `hutool-cron` | `hitool-cron` | **10%** | 2.1 |
| `hutool-crypto` | `hitool-crypto` | 25% | 4 |
| `hutool-db` | `hitool-db` | 15% | 2.2 |
| `hutool-dfa` | `hitool-dfa` | 70% | 4 |
| `hutool-extra` | `hitool-extra` | **5%** | 2.3 + 4.x |
| `hutool-http` | `hitool-http` | 70% | 1.5 + 5 |
| `hutool-json` | `hitool-json` | 85% | 4 |
| `hutool-jwt` | `hitool-jwt` | 95% | 5 |
| `hutool-log` | `hitool-log` | 30% | 2.4 |
| **`hutool-poi`** | **`hitool-poi`** | **0%（占位）** | **3.1** |
| `hutool-script` | `hitool-script` | 80% | 5 |
| `hutool-setting` | `hitool-setting` | 95% | 5 |
| `hutool-socket` | `hitool-socket` | 85% | 5 |
| `hutool-system` | `hitool-system` | 95% | 5 |
| `hutool-ai` | `hitool-ai` | 15%（仅 OpenAI） | 3.3 |
| （无 Java 原型） | `hitool-macros` | 100% | 0 |
| **（**`hutool-compat`** 冻结）** | `hitool-compat-hutool` | **5%**（114 行 / 2 facade） | **冻结** |
| （无 Java 原型） | `hitool-test-support` | 100% | 0 |

**加权总体完成度**：约 **30%**（按"已迁移等价方法数 / Hutool 总方法数"算的更保守口径）；详细文件级缺口见 §10.0 表。

---

## 附录 D：DDD4J → HiTool-Rs 完整生态映射

> 本附录提供 DDD4J（Dromara Distributed Domain for Java）650 个 Java 组件到
> Rust 生态 + HiTool-Rs 的**逐项映射**，作为 §9.1 和 §10 Phase 7 的查询手册。

### D.1 序列化 / 数据格式

| Java 组件 | Rust crate | HiTool 现状 | 建议 |
|---|---|---|---|
| Jackson | `serde_json` | ✅ hitool-json | — |
| Gson | `serde_json` | ✅ 委托 | — |
| FastJSON | `serde_json` | ✅ 替代 | — |
| SnakeYAML | `serde_yaml_ng` | ⚠️ workspace 已声明，hitool-setting 未启用 | **Phase 7.1** 启用 |
| JAXB | `quick-xml` + `serde` | ⚠️ hitool-jaxb_util 占位 | Phase 4 补全 |
...
| Java 组件 | Rust crate | HiTool 现状 | 建议 |
|---|---|---|---|
| OkHttp / Apache HttpClient | `reqwest` | ✅ hitool-http | — |
| Retrofit | `reqwest` + builder | 🟡 hitool-http 已自建 builder | 文档化推荐用法 |
| Spring MVC | `axum` / `actix-web` / `poem` | ❌ 无 | **新建 hitool-http-server** |
...
| PageHelper | `hitool-db::page` | 🟡 部分 | Phase 2.2 补全 |

### D.4 加密与安全

| Java 组件 | Rust crate | HiTool 现状 | 建议 |
|---|---|---|---|
| JCA (javax.crypto) | `RustCrypto`（aes/chacha/des/rc4/sm2/sm3/sm4） | ✅ hitool-crypto | — |
...
| Java 组件 | Rust crate | HiTool 现状 | 建议 |
|---|---|---|---|
| Caffeine | `moka` | ✅ hitool-cache | — |
| Ehcache | `moka` | ✅ | — |
| Guava Cache | `moka` | ✅ | — |
| Redis (Jedis / Lettuce) | `redis` | ⚠️ workspace 未启用 | **Phase 7.2** 新建 hitool-cache-redis |
| Hazelcast | ❌ | — | 不实现（集群方案） |
...
| Java 组件 | Rust crate | HiTool 现状 | 建议 |
|---|---|---|---|
| javax.mail (SMTP) | `lettre` | ✅ hitool-extra::mail（feature-gated） | — |
...
| Java 组件 | Rust crate | HiTool 现状 | 建议 |
|---|---|---|---|
| Quartz | `cron`（workspace 引入） + 自研解析 | ✅ hitool-cron | — |
...
| Java 组件 | Rust crate | HiTool 现状 | 建议 |
|---|---|---|---|
| Logback | `tracing` + `tracing-subscriber` | ✅ hitool-log | — |
...
| Java 组件 | Rust crate | HiTool 现状 | 建议 |
|---|---|---|---|
| Commons Lang | 标准库 + `hitool-core` | ✅ | — |
| Commons Collections | `itertools` + `hitool-core` | 🟡 加 `itertools` | **Phase 7.1** |
| Commons Math | `num` + `statrs` | ❌ | **Phase 7.1** 新增 |
| ICU4J | `unicode-general-category` + `unicode-normalization` | ✅ | — |
| Joda-Time | `chrono` | ✅ | — |
| Commons CSV | `csv` | ✅ hitool-core | — |
...
| Java 组件 | Rust crate | HiTool 现状 | 建议 |
|---|---|---|---|
| Java ImageIO | `image` | ✅ hitool-core::img / hitool-extra::image | — |
| Apache POI / EasyExcel | `rust_xlsxwriter` + `quick-xml` + `zip` | ⚠️ workspace 已声明，hitool-poi crate **待建** | **Phase 3.1 占位** |
| iText / Apache PDFBox | `printpdf` / `lopdf` | ❌ | Phase 4 可选 |
| zxing (QR) | `qrcode`（svg feature） | ✅ hitool-extra::qrcode | — |
...
### D.16 不实现项（DDD4J 推荐但与 hitool-rs 原则冲突）

| 项 | 原因 |
|---|---|
| ❌ Spring 全家桶（Spring Boot/Data Cloud/Security） | Rust 无 Spring，框架职责由应用层承担 |
| ❌ MyBatis / Hibernate / JPA | 违反「不造 ORM」原则 |
| ❌ Tomcat / Jetty / Undertow Web 容器 | hitool-http 是客户端；Server 应新建独立 crate |
| ❌ Swing / JavaFX 桌面 UI | hitool-core::swing 仅做底层截屏/键鼠，不做 UI 框架 |
| ❌ JNDI | 无容器概念，改用显式注入 |
| ❌ RMI / CORBA / JNI | Rust 有更好的 IPC 机制 |

---

## 附录 E：hutool 全模块文件 ↔ hitool-rs 1:1 对照清单（**用户硬性要求验收清单**）

> 本附录是 §5.0.1 「1:1 全文件覆盖硬性要求」的**具体可验收清单**。
> 每个 hutool Java `.java` 文件都必须有对应的 hitool-rs `.rs` 文件（除 hutool-poi）。

### E.1 hutool-core（713 .java → target 713+ .rs）

| Java 包 | Java 文件数 | hitool 子目录 | 当前 .rs | 缺口 | Phase |
|---|---:|---|---:|---:|---|
...
**hutool-core 完成度：≥ 100%（含 hitool 扩展）**

### E.2 hutool-cron（41 .java → target 41 .rs）

| Java 文件 | Rust 目标 | 当前 | Phase |
|---|---|---|---|
| `cn.hutool.cron.CronUtil` | `hitool-cron/src/lib.rs` | 🟡 部分 | Phase 2.1 |
| `cn.hutool.cron.CronPattern` | `hitool-cron/src/cron_pattern.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.CronPatternBuilder` | `hitool-cron/src/cron_pattern_builder.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.CronPatternUtil` | `hitool-cron/src/cron_pattern_util.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.CronTask` | `hitool-cron/src/cron_task.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.CronTimer` | `hitool-cron/src/cron_timer.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.CronConfig` | `hitool-cron/src/cron_config.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.CronException` | `hitool-cron/src/cron_exception.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.SystemTimer` | `hitool-cron/src/system_timer.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.Scheduler` | `hitool-cron/src/scheduler.rs` | 🟡 | Phase 2.1 |
| `cn.hutool.cron.Task` | `hitool-cron/src/task.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.TaskExecutor` | `hitool-cron/src/task_executor.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.TaskExecutorManager` | `hitool-cron/src/task_executor_manager.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.TaskLauncher` | `hitool-cron/src/task_launcher.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.TaskLauncherManager` | `hitool-cron/src/task_launcher_manager.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.TaskListener` | `hitool-cron/src/task_listener.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.TaskListenerManager` | `hitool-cron/src/task_listener_manager.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.TaskTable` | `hitool-cron/src/task_table.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.TimerTask` | `hitool-cron/src/timer_task.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.TimerTaskList` | `hitool-cron/src/timer_task_list.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.pattern.Part` | `hitool-cron/src/pattern/part.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.pattern.PartMatcher` | `hitool-cron/src/pattern/part_matcher.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.pattern.PartParser` | `hitool-cron/src/pattern/part_parser.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.pattern.PatternMatcher` | `hitool-cron/src/pattern/pattern_matcher.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.pattern.PatternParser` | `hitool-cron/src/pattern/pattern_parser.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.pattern.PatternUtil` | `hitool-cron/src/pattern/pattern_util.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.pattern.matcher.AlwaysTrueMatcher` | `hitool-cron/src/pattern/matcher/always_true_matcher.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.pattern.matcher.BoolArrayMatcher` | `hitool-cron/src/pattern/matcher/bool_array_matcher.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.pattern.matcher.DayOfMonthMatcher` | `hitool-cron/src/pattern/matcher/day_of_month_matcher.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.pattern.matcher.YearValueMatcher` | `hitool-cron/src/pattern/matcher/year_value_matcher.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.task.InvokeTask` | `hitool-cron/src/task/invoke_task.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.task.RunnableTask` | `hitool-cron/src/task/runnable_task.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.timingwheel.TimingWheel` | `hitool-cron/src/timingwheel/timing_wheel.rs` | ❌ | Phase 2.1 |
| `cn.hutool.cron.listener.SimpleTaskListener` | `hitool-cron/src/listener/simple_task_listener.rs` | ❌ | Phase 2.1 |
...
| 其他 + package-info | ~23 | 32（hitool-db 当前） | Phase 2.2 |
...
| Java 文件 | Rust 目标 | 当前 |
|---|---|---|
| `cn.hutool.dfa.SensitiveUtil` | `hitool-dfa/src/sensitive_util.rs` | ❌ |
| `cn.hutool.dfa.WordTree` | `hitool-dfa/src/word_tree.rs` | 🟡 部分 |
| `cn.hutool.dfa.FoundWord` | `hitool-dfa/src/found_word.rs` | ❌ |
| `cn.hutool.dfa.SensitiveProcessor` | `hitool-dfa/src/sensitive_processor.rs` | ❌ |
| `cn.hutool.dfa.StopChar` | `hitool-dfa/src/stop_char.rs` | ❌ |
| `cn.hutool.dfa.package-info` | `hitool-dfa/src/lib.rs` | 🟡 |
...
find "$HITOOL_ROOT/crates" -name "*.rs" \
  -not -path "*/target/*" -not -path "*/tests/*" -not -path "*/examples/*" \
  -not -path "*/hitool-poi/*" \
  | sed 's|.*/crates/hitool-||' | sed 's|/src/.*/|/|' | sed 's|.rs$||' \
  | sort -u > /tmp/hitool_files.txt
...
**v1.0.0 准入条件**：除 hutool-poi 外，所有 hutool 模块的 Java 文件都必须在 hitool-rs 中有对应 `.rs` 文件。
...
2. ✅ hitool-rs 当前迁移进度审计（926 Rust 文件 vs 1553 Java 文件，**完成度 59.6% 文件数 / 50% 方法数**）
3. ✅ hitool-core 双重路径问题识别与处理策略
...
6. ✅ hitool-ai 7 provider 补全计划
7. ✅ hitool-cron / hitool-db / hitool-extra 大件补全方案
...
**下一步**：批准后立即执行 Phase 0 Step 1（建立文档体系）和 Step 2（创建 hitool-poi 占位骨架）。