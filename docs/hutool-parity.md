...
| `hutool-core` | `hitool-core` | idiomatic | strings, collections, codecs, dates, IDs, closure/Serde-backed builders, ownership-aware mutable wrappers, Hutool boolean vocabulary/aggregations, checked endian-aware `ByteUtil`, `encoding_rs`-backed charset conversion/detection, GB 32100 credit codes, owned/range-based pagination, bounded custom radix conversion, Java UTF-16 classic hashes, and a `hex`/`num-bigint`-backed `HexUtil` facade |
