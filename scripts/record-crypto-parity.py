#!/usr/bin/env python3
"""Record Hutool crypto APIs against hutool-crypto idiomatic surfaces / planned gaps."""
...
def mapping(class_name: str, method: str | None, signature: str) -> tuple[str, str, str, str] | None:
    ...
    if class_name in digest:
        return (
            "idiomatic",
            "hutool_crypto::{DigestUtil,Digester,Md5Util,Sm3Util,md5_hex,sha256_hex,sm3_hex}",
            "crates/hutool-crypto/tests/crypto_parity_extended.rs::digest_hash256_test",
            "Hutool digest facades map to typed hex helpers; SecureUtil-style digests share the same vectors.",
        )
    if class_name in aes:
        return (
            "idiomatic",
            "hutool_crypto::{Aes,aes256_gcm_encrypt,aes128_cbc_encrypt,aes128_ecb_encrypt}",
            "crates/hutool-crypto/tests/crypto_util_parity.rs::aes256_gcm_encrypt_decrypt",
    ...
    if class_name == "SecureUtil":
        return (
            "idiomatic",
            "hutool_crypto::{sha256_hex,hmac_sha256,aes256_gcm_encrypt,hash_password,pbkdf2_hex}",
            "crates/hutool-crypto/tests/crypto_parity_extended.rs::secure_util_sha256_test",
            "SecureUtil static facade is split into typed Rust helpers (digest/MAC/AEAD/password/PBKDF2).",
        )
    if class_name in keys:
        return (
            "idiomatic",
            "hutool_crypto::{generate_random_key_bytes,generate_rsa_keypair,generate_ec_keypair,generate_sm2_keypair}",
            "crates/hutool-crypto/tests/crypto_parity_gap.rs::key_util_generate_key_pair_test",
            "Key generation uses RustCrypto/OS RNG instead of BouncyCastle KeyPairGenerator.",
        )
    if class_name in rsa:
        return (
            "idiomatic",
            "hutool_crypto::{Rsa,SignUtil,generate_rsa_keypair,rsa_public_from_private_key,KeyType}",
            "crates/hutool-crypto/tests/crypto_parity_extended.rs::rsa_test",
            "RSA/asymmetric signing helpers are owned Rust types; Java Key/Certificate overloads collapse to byte/PEM inputs.",
        )
    if class_name in sm:
        return (
            "idiomatic",
            "hutool_crypto::{Sm4,sm2_sign,sm2_verify,sm4_ecb_encrypt,generate_sm2_keypair}",
            "crates/hutool-crypto/tests/crypto_parity_extended.rs::sm2_test",
            "SM2/SM4 use Rust SM crates; Hutool BC provider wiring is omitted.",
        )
    if class_name in mac:
        return (
            "idiomatic",
            "hutool_crypto::{HMac,hmac_sha256,hmac_sha256_hex,hmac_md5_hex,hmac_sha1_hex,hmac_sm3_hex}",
            "crates/hutool-crypto/tests/crypto_util_parity.rs::hmac_sha256_basic",
            "MAC engines collapse to typed HMAC helpers with explicit key/message bytes.",
        )
    if class_name in otp:
        return (
            "idiomatic",
            "hutool_crypto::{Hotp,Totp,hotp,totp,totp_validate,decode_base32_secret}",
            "crates/hutool-crypto/tests/crypto_parity_extended.rs::otp_gen_key_test",
            "HOTP/TOTP match Hutool counter/time-step semantics via Rust OTP helpers.",
        )
    if class_name == "PemUtil":
        return (
            "idiomatic",
            "hutool_crypto::{read_pem_key,read_pem_private_key,write_pkcs8_private_pem,PemKind}",
            "crates/hutool-crypto/tests/crypto_parity_extended.rs::pem_read_private_key_test",
            "PEM read/write is pure Rust without BouncyCastle PEMParser.",
        )
    if class_name in password:
        return (
            "idiomatic",
            "hutool_crypto::{hash_password,verify_password}",
            "crates/hutool-crypto/tests/crypto_util_parity.rs::bcrypt_hash_and_verify",
            "Password hashing uses Argon2id (SecretString) rather than Java BCrypt/Argon2 SPI beans.",
        )
    if class_name == "ChaCha20":
        return (
            "idiomatic",
            "hutool_crypto::{chacha20_encrypt,chacha20_decrypt}",
            "crates/hutool-crypto/tests/crypto_parity_extended.rs::chacha20_encrypt_decrypt_test",
            "ChaCha20 is a typed helper over RustCrypto stream cipher primitives.",
        )
    if class_name in legacy:
        return (
            "idiomatic",
            "hutool_crypto::{des_ecb_encrypt,Rc4,FpeFf1,vigenere_encrypt,tea_encrypt}",
            "crates/hutool-crypto/tests/crypto_parity_extended.rs::symmetric_des_test",
            "Legacy DES/RC4/FPE/Vigenere/TEA keep Hutool-shaped helpers with explicit Rust ownership.",
        )
    if class_name == "PBKDF2":
        return (
            "idiomatic",
            "hutool_crypto::{pbkdf2_hex,pbkdf2_sha1_hex}",
            "crates/hutool-crypto/tests/crypto_parity_extended.rs::pbkdf2_encrypt_test",
            "PBKDF2-HMAC-SHA1 matches Hutool SecureUtil.pbkdf2 hex width.",
        )
    if class_name == "CryptoException":
        return (
            "idiomatic",
            "hutool_crypto::CryptoError",
            "crates/hutool-crypto/tests/crypto_util_parity.rs::aes256_gcm_encrypt_decrypt",
    ...
    if class_name == "ASN1Util":
        return (
            "idiomatic",
            "hutool_crypto::Asn1Util",
            "crates/hutool-crypto/tests/crypto_parity_facade.rs::asn1_util_encode_decode",
            "ASN1Util DER SEQUENCE helpers use the `der` crate; BC ASN1Encodable types collapse to octet payloads.",
        )
    if class_name == "SpecUtil":
        return (
            "idiomatic",
            "hutool_crypto::{SpecUtil,KeySpecBytes,PbeKeySpec,RsaPrivateCrtKeySpec}",
            "crates/hutool-crypto/tests/crypto_parity_facade.rs::spec_util_key_and_xml",
            "Java KeySpec/PBE*Spec/RSAPrivateCrtKeySpec map to owned Rust byte/BigUint containers.",
        )
    if class_name == "ECIES":
        return (
            "idiomatic",
            "hutool_crypto::{Ecies,encrypt_ecies,decrypt_ecies}",
            "crates/hutool-crypto/tests/crypto_parity_facade.rs::ecies_encrypt_decrypt_roundtrip",
...
def bc_util_mapping(method: str | None, signature: str) -> tuple[str, str, str, str] | None:
    ...
    idiomatic = (
        "idiomatic",
        "hutool_crypto::{BcUtil,EcDomainParams,EcPrivateParams,EcPublicParams}",
        "crates/hutool-crypto/tests/crypto_parity_facade.rs::bc_util_ec_and_sm2_params",
...
def planned_note(class_name: str, method: str | None, signature: str) -> str:
    ...
    if class_name == "CipherWrapper":
        return (
            ...
            "no JCA Cipher mirror in hutool-crypto — kept planned."
        )
    ...
    return (
        f"{class_name} not yet ported as a first-class hutool-crypto surface; "
