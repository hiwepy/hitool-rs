...
def evidence(qualified_name: str) -> tuple[str, str]:
    ...
    if member in {"bodyBytes", "charset", "httpVersion", "toString"}:
        return (
            "crates/hitool-http/src/base.rs::version_body_charset_and_display_match_hutool_metadata_semantics",
    ...
    return (
        "crates/hitool-http/src/base.rs::headers_are_case_insensitive_mutable_and_wire_aggregation_is_explicit",
...
def main() -> None:
    ...
    for row in inventory:
        ...
        indexed[row["api_id"]] = {
            "api_id": row["api_id"],
            "status": "idiomatic",
            "hitool_symbol": "hitool_http::HttpBase",
            "test_evidence": test,
            "notes": notes,
        }
    if selected != 25:
